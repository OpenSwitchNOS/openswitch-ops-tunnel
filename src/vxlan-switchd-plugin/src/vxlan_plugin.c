            ...
            if (old_key != new_key) {
                if (old_key) {
                    ...
                    if (plugin->vport_unbind_all_ports_on_vlan) {
                        asic_plugin->vport_unbind_all_ports_on_vlan(old_key,
                                                                    node->vlan_id);
                    } else {
                ...
                if (new_key) {
                    ...
                    if (plugin->vport_bind_all_ports_on_vlan) {
                        ...
                        asic_plugin->vport_bind_all_ports_on_vlan(new_key,
                                                                  node->vlan_id);
                    } else {
...
static void vxlan_bind_port_vlan(const char *vlan_name, struct port *port)
{
    ...
    if (plugin->vport_bind_port_on_vlan) {
        asic_plugin->vport_bind_port_on_vlan(tunnel_key,
                                             vlan_node->vlan_id,
                                             port);
    } else {
...
static void vxlan_unbind_port_vlan(const char *vlan_name, struct port *port)
{
    ...
    if (plugin->vport_unbind_port_on_vlan) {
        asic_plugin->vport_unbind_port_on_vlan(tunnel_key,
                                               vlan_node->vlan_id,
                                               port);
    } else {
