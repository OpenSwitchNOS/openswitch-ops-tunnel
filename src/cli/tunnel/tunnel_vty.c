...
char * add_vni_to_vni_list(const char *cur_vni_list, const char *vni)
{
    ...
    copy_vni_list = xmalloc(TUNNEL_MAX_VNIS_IN_VNI_LIST);
    memset(copy_vni_list, 0, TUNNEL_MAX_VNIS_IN_VNI_LIST);
    snprintf(copy_vni_list, TUNNEL_MAX_VNIS_IN_VNI_LIST, "%s", cur_vni_list);

    new_vni_list = xmalloc(TUNNEL_MAX_VNIS_IN_VNI_LIST);
    memset(new_vni_list, 0, TUNNEL_MAX_VNIS_IN_VNI_LIST);
...
char * remove_vni_to_vni_list(const char *cur_vni_list, const char *vni)
{
    ...
    if(temp_vni_list != NULL) {
        vni_str = strtok(temp_vni_list, " ");
        if (strcmp(vni_str, vni) != 0)
            snprintf(new_vni_list, TUNNEL_MAX_VNIS_IN_VNI_LIST, "%s", vni_str);
        else
