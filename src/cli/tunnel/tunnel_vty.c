...
char * add_vni_to_vni_list(const char *cur_vni_list, const char *vni)
{
    ...
    copy_vni_list = xmalloc(sizeof(int64_t) * sizeof(char) * 8000);
    memset(copy_vni_list, 0, (sizeof(int64_t) * sizeof(char) * 8000));
    snprintf(copy_vni_list, (sizeof(int64_t) * sizeof(char) * 8000),
            "%s", cur_vni_list);
...
char * remove_vni_to_vni_list(const char *cur_vni_list, const char *vni)
{
    ...
    if(temp_vni_list != NULL)
    {
        ...
        if (strcmp(vni_str, vni) != 0)
        {
            snprintf(new_vni_list, (sizeof(int64_t) * sizeof(char) * 8000),
            "%s", vni_str);
        }
        else
    ...
    if (new_vni_list[strlen(new_vni_list) - 1] == ' ')
        ...
        new_vni_list[strlen(new_vni_list)] = '\0';
    vty_out(vty, "new_vni_list %s!!! %s", new_vni_list, VTY_NEWLINE);
    return new_vni_list;
}
